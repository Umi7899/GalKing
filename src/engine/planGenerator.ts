...
import { getLesson, getGrammarPoint, getVocabPack, getSentencesByGrammar } from '../db/queries/content';
...
async function findCurrentGrammar(
    ...
    if (!lesson || lesson.grammarIds.length === 0) {
        ...
        const firstLesson = await getLesson(1);
...
async function generateStep4(grammarId: number, level: number): Promise<Step4State> {
    // Get Gal-style sentences for this grammar
    const sentences = await getSentencesByGrammar(grammarId, 'gal');

    // Filter by level (±1)
    const eligible = sentences.filter(s => Math.abs(s.level - level) <= 1);
    ...
    if (type === 'counter') {
        ...
        if (counterExample) {
            return {
                ...
                stem: '以下哪个句子是错误的用法？',
                options: [
                    { id: 'a', text: counterExample.jp },
                    { id: 'b', text: grammar.examples[0]?.jp ?? '正确例句' },
                    { id: 'c', text: grammar.examples[1]?.jp ?? '另一正确例句' },
                    { id: 'd', text: grammar.examples[2]?.jp ?? '第三正确例句' },
                ],
                correctId: 'a',
